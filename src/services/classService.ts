...
export class ClassService extends BaseService {
  ...
  async assignSubjectToClass(classId: string, subjectId: string, teacherId?: string) {
    ...
    return await this.executeTransaction(async (client) => {
      ...
      const subjectExists = await client.query(
      ...
      // Create assignment
      const result = await client.query(
        `INSERT INTO class_subjects (class_id, subject_id, teacher_id, created_at, updated_at)
         VALUES ($1, $2, $3, NOW(), NOW())
         RETURNING id, class_id, subject_id, teacher_id, created_at, updated_at`,
        [actualClassId, subjectId, teacherId || null]
      );
  ...
  async removeSubjectFromClass(classId: string, subjectId: string) {
    ...
    const assignmentExists = await this.executeQuery(
      'SELECT id FROM class_subjects WHERE class_id = $1 AND subject_id = $2',
      [actualClassId, subjectId]
    );

    if (assignmentExists.rows.length === 0) {
      throw new AppError('Subject is not assigned to this class', 404);
    }

    // Check for dependencies (grades)
    const gradesCheck = await this.executeQuery(
      'SELECT COUNT(*) as count FROM grades WHERE class_id = $1 AND subject_id = $2',
      [actualClassId, subjectId]
    );

    if (parseInt(gradesCheck.rows[0].count) > 0) {
      throw new AppError('Cannot remove subject. It has associated grade records.', 409);
    }
   ...
   */
  async enrollStudentToClass(classId: string, studentId: string) {
    const classInfo = await this.checkEntityExists('classes', classId, 'alt_id');
    const actualClassId = classInfo.id;
    ...
    return await this.executeTransaction(async (client) => {
      ...
      const studentExists = await client.query(
        'SELECT id, user_id FROM students WHERE id = $1 AND is_active = true',
        [studentId]
      );
      ...
      // Enroll student
      await client.query(
        'UPDATE students SET class_id = $1, enrollment_date = NOW(), updated_at = NOW() WHERE id = $2',
        [actualClassId, studentId]
      );
   ...
   */
  async bulkEnrollStudentsToClass(classId: string, studentIds: string[]) {
    const classInfo = await this.checkEntityExists('classes', classId, 'alt_id');
    const actualClassId = classInfo.id;
    ...
    return await this.executeTransaction(async (client) => {
      ...
      if (studentIds.length > availableSpots) {
        throw new AppError(`Class has only ${availableSpots} available spots, but ${studentIds.length} students requested`, 409);
      }
      ...
      if (results.length > 0) {
        await client.query(
          'UPDATE classes SET current_enrollment = current_enrollment + $1, updated_at = NOW() WHERE id = $2',
          [results.length, actualClassId]
        );
      }
   ...
   */
  async transferStudent(studentId: string, targetClassId: string) {
    return await this.executeTransaction(async (client) => {
      // Check if student exists and is active
      const studentExists = await client.query(
        'SELECT id, class_id, user_id FROM students WHERE id = $1 AND is_active = true',
        [studentId]
      );
      ...
      const currentClassInfo = await client.query(
        'SELECT name, grade, section FROM classes WHERE id = $1',
        [currentClassId]
      );

      const targetClassInfo = await client.query(
        'SELECT name, grade, section FROM classes WHERE id = $1',
        [actualTargetClassId]
      );
  ...
  async getClassSubjects(classId: string) {
    ...
    const subjectsResult = await this.executeQuery(
      `SELECT cs.id as assignment_id, cs.teacher_id,
