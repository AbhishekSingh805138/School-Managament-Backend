...
export class FeeService extends BaseService {
  ...
  async getFeeCategories(req: any) {
    ...
    const countResult = await this.executeQuery(
      `SELECT COUNT(*) FROM fee_categories fc
  ...
  async getFeeCategoryById(id: string) {
    ...
    // Get students assigned to this fee category
  ...
  async updateFeeCategory(id: string, updateData: UpdateFeeCategory) {
    ...
    // Check if name is being updated and conflicts with another category
    ...
    return await this.executeTransaction(async (client) => {
      ...
      for (const studentId of assignmentData.studentIds) {
        ...
        const studentExists = await client.query(
          'SELECT id, student_id FROM students WHERE id = $1 AND is_active = true',
          [studentId]
        );

        if (studentExists.rows.length === 0) {
          throw new AppError(`Student with ID ${studentId} not found or inactive`, 404);
        }

        // Check if fee already assigned to this student
        const existingAssignment = await client.query(
          'SELECT id FROM student_fees WHERE student_id = $1 AND fee_category_id = $2',
          [studentId, assignmentData.feeCategoryId]
        );
        ...
        const result = await client.query(
          `INSERT INTO student_fees (student_id, fee_category_id, amount, due_date, status)
