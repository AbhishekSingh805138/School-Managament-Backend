...
export class PaymentService extends BaseService {
  async recordPayment(paymentData: CreatePayment, processedBy: string) {
    ...
    const studentFeeResult = await this.executeQuery(
      `SELECT sf.id, sf.student_id, sf.fee_category_id, sf.amount, sf.status,
    ...
    const paidAmountResult = await this.executeQuery(
    ...
    return await this.executeTransaction(async (client) => {
      ...
      const paymentResult = await client.query(
        `INSERT INTO payments (student_fee_id, amount, payment_date, payment_method, transaction_id, receipt_number, processed_by)
      ...
      await client.query(
        ...
        [newStatus, paymentData.studentFeeId]
      );
  ...
  private async executePaymentsQuery(req: any) {
    ...
    const result = await this.executeQuery(
      `SELECT p.id, p.student_fee_id, p.amount, p.payment_date, p.payment_method, p.transaction_id, 
  ...
  private async generateReceiptNumber(): Promise<string> {
    ...
    const countResult = await this.executeQuery(
      'SELECT COUNT(*) as count FROM payments WHERE DATE(created_at) = CURRENT_DATE'
    );
