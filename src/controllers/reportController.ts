...
export const generateAttendanceReport = asyncHandler(async (req: Request, res: Response) => {
  const reportQuery = AttendanceReportQuerySchema.parse(req.query);
  ...
  const reportResponse = formatReportResponse(
    'attendance',
    'Attendance Report',
    reportQuery,
    reportData,
...
export const generateAcademicReport = asyncHandler(async (req: Request, res: Response) => {
  const reportQuery = AcademicReportQuerySchema.parse(req.query);
  const userId = req.user!.id;
  const userRole = req.user!.role;
...
export const generateFinancialReport = asyncHandler(async (req: Request, res: Response) => {
  const reportQuery = FinancialReportQuerySchema.parse(req.query);
  const userId = req.user!.id;
  const userRole = req.user!.role;
...
export const generateEnrollmentReport = asyncHandler(async (req: Request, res: Response) => {
  const reportQuery = EnrollmentReportQuerySchema.parse(req.query);
  const userId = req.user!.id;
  const userRole = req.user!.role;

  // Only admins and staff can access enrollment reports
  if (!['admin', 'staff'].includes(userRole)) {
    throw new AppError('You are not authorized to access enrollment reports', 403);
  }
...
export const generateDashboardReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Only admins can access dashboard reports
  if (userRole !== 'admin') {
    throw new AppError('Only administrators can access dashboard reports', 403);
  }
  ...
  const academicStats = await query(`
    SELECT 
  ...
  const dashboardData = {
    ...
    recent: {
      newEnrollments: recentEnrollments.rows.map((row: any) => ({
        studentName: row.student_name,
        className: row.class_name,
        enrollmentDate: row.enrollment_date.toISOString().split('T')[0],
      })),
      recentPayments: recentPayments.rows.map((row: any) => ({
        studentName: row.student_name,
        amount: parseFloat(row.amount),
        paymentDate: row.payment_date.toISOString().split('T')[0],
      })),
      upcomingEvents: [], // This would be populated from an events system if implemented
    },
  };
...
async function generateStudentAttendanceReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
...
async function generateClassAttendanceReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
       c.id as class_id,
       c.name as class_name,
...
async function generateDateAttendanceReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
       a.date,
...
// Helper functions for academic reports
async function generateStudentAcademicReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
       s.id as student_id,
       s.student_id as student_number,
...
async function generateSubjectAcademicReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
...
async function generateClassAcademicReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
       c.id as class_id,
       c.name as class_name,
...
async function calculateAcademicSummary(whereClause: string, queryParams: any[]): Promise<any> {
  const result = await query(
    `SELECT 
       COUNT(DISTINCT s.id) as total_students,
       COUNT(g.id) as total_assessments,
...
async function generateStudentFinancialReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
       s.id as student_id,
       s.student_id as student_number,
...
async function generateClassFinancialReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
       c.id as class_id,
       c.name as class_name,
...
async function generateCategoryFinancialReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
...
async function calculateFinancialSummary(whereClause: string, queryParams: any[]): Promise<any> {
  const result = await query(
    `SELECT 
       COUNT(DISTINCT s.id) as total_students,
       SUM(sf.amount) as total_fees,
...
// Helper functions for enrollment reports
async function generateClassEnrollmentReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
       c.id as class_id,
       c.name as class_name,
...
async function generateGradeEnrollmentReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
       c.grade,
...
async function generateMonthlyEnrollmentReport(whereClause: string, queryParams: any[], reportQuery: any): Promise<any[]> {
  const result = await query(
    `SELECT 
  ...
  return result.rows.map((row: any) => ({
    enrollmentMonth: row.enrollment_month.toISOString().slice(0, 7), // YYYY-MM format
    newEnrollments: parseInt(row.new_enrollments),
    classesAffected: parseInt(row.classes_affected),
  }));
}

async function calculateEnrollmentSummary(whereClause: string, queryParams: any[]): Promise<any> {
  const result = await query(
    `SELECT 
