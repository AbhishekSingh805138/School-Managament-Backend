...
export const createFeeCategory = asyncHandler(async (req: Request, res: Response) => {
  const feeCategoryData: CreateFeeCategory = req.body;

  // Validate academic year exists
  const academicYearExists = await query(
    'SELECT id, name FROM academic_years WHERE id = $1 AND is_active = true',
    [feeCategoryData.academicYearId]
  );

  if (academicYearExists.rows.length === 0) {
    throw new AppError('Academic year not found or inactive', 404);
  }

  // Check if fee category name already exists for this academic year
  const existingCategory = await query(
    'SELECT id FROM fee_categories WHERE name = $1 AND academic_year_id = $2',
    [feeCategoryData.name, feeCategoryData.academicYearId]
  );
  ...
  // Generate sequential ID for fee category alt_id
  const seqIdResult = await query('SELECT generate_sequential_id($1) as next_id', ['fee_categories']);
  const sequentialId = seqIdResult.rows[0].next_id;

  // Create fee category
  const result = await query(
    `INSERT INTO fee_categories (name, description, amount, frequency, is_mandatory, academic_year_id, alt_id)
...
export const getFeeCategories = asyncHandler(async (req: Request, res: Response) => {
  ...
  const countResult = await query(
    `SELECT COUNT(*) FROM fee_categories fc ${whereClause}`,
    queryParams
  );
  ...
  // Get fee categories
  const result = await query(
    `SELECT fc.id, fc.alt_id, fc.name, fc.description, fc.amount, fc.frequency, 
...
export const getFeeCategoryById = asyncHandler(async (req: Request, res: Response) => {
  ...
  const assignedStudentsResult = await query(
    'SELECT COUNT(*) as assigned_count FROM student_fees WHERE fee_category_id = $1',
    [id]
  );
...
export const updateFeeCategory = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const updateData: UpdateFeeCategory = req.body;

  // Check if fee category exists
  const existingCategory = await query(
    'SELECT id, academic_year_id FROM fee_categories WHERE id = $1',
    [id]
  );
  ...
  // Check if name already exists for this academic year (if name is being updated)
  if (updateData.name) {
    const nameExists = await query(
      'SELECT id FROM fee_categories WHERE name = $1 AND academic_year_id = $2 AND id != $3',
      [updateData.name, existingCategory.rows[0].academic_year_id, id]
    );
...
export const deleteFeeCategory = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Check if fee category exists
  const existingCategory = await query(
    'SELECT id FROM fee_categories WHERE id = $1',
    [id]
  );
  ...
  // Check if there are any student fees associated with this category
  const associatedFees = await query(
    'SELECT COUNT(*) as count FROM student_fees WHERE fee_category_id = $1',
    [id]
  );
  ...
  if (feeCount > 0) {
    throw new AppError(
      `Cannot delete fee category. It has ${feeCount} associated student fees. Please deactivate instead.`,
      409
    );
  }
...
export const assignFeesToStudents = asyncHandler(async (req: Request, res: Response) => {
  const assignmentData: AssignFeesToStudents = req.body;

  // Validate fee category exists
  const feeCategoryExists = await query(
    'SELECT id, name, amount FROM fee_categories WHERE id = $1 AND is_active = true',
    [assignmentData.feeCategoryId]
  );

  if (feeCategoryExists.rows.length === 0) {
    throw new AppError('Fee category not found or inactive', 404);
  }
  ...
  const studentsValidation = await query(
    `SELECT s.id, s.student_id, u.first_name, u.last_name 
     ...
     WHERE s.id = ANY($1) AND s.is_active = true AND u.is_active = true`,
    [assignmentData.studentIds]
  );

  if (studentsValidation.rows.length !== assignmentData.studentIds.length) {
    throw new AppError('Some students are not found or inactive', 400);
  }

  // Check for existing assignments
  const existingAssignments = await query(
    'SELECT student_id FROM student_fees WHERE student_id = ANY($1) AND fee_category_id = $2',
    [assignmentData.studentIds, assignmentData.feeCategoryId]
  );

  if (existingAssignments.rows.length > 0) {
    const existingStudentIds = existingAssignments.rows.map((row: any) => row.student_id);
    throw new AppError(`Some students already have this fee assigned: ${existingStudentIds.join(', ')}`, 409);
  }
  ...
  try {
    ...
    for (const studentId of assignmentData.studentIds) {
      // Generate sequential ID for student fee alt_id
      const seqIdResult = await client.query('SELECT generate_sequential_id($1) as next_id', ['student_fees']);
      const sequentialId = seqIdResult.rows[0].next_id;

      const result = await client.query(
        `INSERT INTO student_fees (student_id, fee_category_id, amount, due_date, discount_amount, alt_id)
         VALUES ($1, $2, $3, $4, $5, $6)
...
export const assignFeesToClass = asyncHandler(async (req: Request, res: Response) => {
  const assignmentData: AssignFeesToClass = req.body;
  ...
  // Validate class exists and get all students
  const classStudents = await query(
    `SELECT s.id, s.student_id, u.first_name, u.last_name, c.name as class_name
     FROM students s 
     JOIN users u ON s.user_id = u.id 
     JOIN classes c ON s.class_id = c.id
     WHERE s.class_id = $1 AND s.is_active = true AND u.is_active = true AND c.is_active = true`,
  ...
  const studentIds = classStudents.rows.map((student: any) => student.id);

  // Check for existing assignments
  const existingAssignments = await query(
    'SELECT student_id FROM student_fees WHERE student_id = ANY($1) AND fee_category_id = $2',
    [studentIds, assignmentData.feeCategoryId]
  );

  if (existingAssignments.rows.length > 0) {
    const existingCount = existingAssignments.rows.length;
    throw new AppError(`${existingCount} students in this class already have this fee assigned`, 409);
  }
...
export const getStudentFees = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Update overdue status for pending fees
  await query(
    `UPDATE student_fees 
  ...
  // Get student fees
  const result = await query(
    `SELECT sf.id, sf.alt_id, sf.student_id, sf.fee_category_id, sf.amount, sf.due_date, 
...
export const getStudentFeeById = asyncHandler(async (req: Request, res: Response) => {
  ...
  const paymentsResult = await query(
    `SELECT p.id, p.amount, p.payment_date, p.payment_method, p.receipt_number, p.remarks,
...
export const updateStudentFee = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const updateData: UpdateStudentFee = req.body;

  // Check if student fee exists
  const existingFee = await query(
    'SELECT id, amount, discount_amount FROM student_fees WHERE id = $1',
    [id]
  );
  ...
  // Validate discount amount doesn't exceed fee amount
  if (updateData.discountAmount !== undefined && updateData.amount !== undefined) {
    ...
    if (updateData.discountAmount > currentAmount) {
      throw new AppError('Discount amount cannot exceed current fee amount', 400);
    }
    ...
    if (currentDiscount > updateData.amount) {
      throw new AppError('Current discount amount exceeds new fee amount', 400);
    }
  }
