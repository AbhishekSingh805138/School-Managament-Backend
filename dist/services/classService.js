...
class ClassService extends baseService_1.BaseService {
    async createClass(classData) {
        return await this.executeTransaction(async (client) => {
         ...
         RETURNING id, alt_id, name, grade, section, teacher_id, capacity, room, description, academic_year_id, current_enrollment, is_active, created_at, updated_at`, [
            ...
            await cacheService_1.default.delPattern(`${cacheService_1.CacheKeys.CLASSES_ALL}*`);
            return {
    ...
    async getClasses(req) {
        ...
        const cacheKey = `${cacheService_1.CacheKeys.CLASSES_ALL}:${page}:${limit}:${sortBy}:${sortOrder}:${isActive || 'all'}:${academicYearId || 'all'}:${grade || 'all'}:${teacherId || 'all'}`;
