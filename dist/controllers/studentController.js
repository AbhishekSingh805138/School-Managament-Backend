...
exports.getStudentSummary = exports.bulkUpdateStudents = exports.getStudentsByClass = exports.getStudentClassHistory = exports.deleteStudent = exports.updateStudent = exports.getStudentById = exports.getStudents = exports.createStudent = void 0;
...
exports.createStudent = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const passwordHash = await (0, auth_1.hashPassword)(studentData.password);
       ...
       FROM classes c WHERE c.id = $2`, [student.id, studentData.classId, studentData.enrollmentDate]);
        const classInfoResult = await client.query(`SELECT c.name, c.grade, c.section, ay.name as academic_year_name
       FROM classes c
       JOIN academic_years ay ON c.academic_year_id = ay.id
       WHERE c.id = $1`, [studentData.classId]);
...
exports.getStudents = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const countResult = await (0, connection_1.query)(`SELECT COUNT(*) FROM students s
     JOIN users u ON s.user_id = u.id
     JOIN classes c ON s.class_id = c.id
     ${whereClause}`, values);
    const total = parseInt(countResult.rows[0].count);
    const result = await (0, connection_1.query)(`SELECT s.id, s.alt_id, s.user_id, s.student_id, s.class_id, s.enrollment_date,
...
exports.getStudentById = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    let result;
    if (isUUID) {
        result = await (0, connection_1.query)(`SELECT s.id, s.alt_id, s.user_id, s.student_id, s.class_id, s.enrollment_date,
              s.guardian_name, s.guardian_phone, s.guardian_email, s.emergency_contact,
              s.medical_info, s.is_active, s.created_at, s.updated_at,
              u.first_name, u.last_name, u.email, u.phone, u.date_of_birth, u.address,
              c.name as class_name, c.grade, c.section,
       ...
       JOIN classes c ON s.class_id = c.id
       JOIN academic_years ay ON c.academic_year_id = ay.id
       WHERE s.alt_id = $1 OR s.id::text = $1 OR s.student_id = $1`, [id]);
    }
    if (result.rows.length === 0) {
        throw new errorHandler_1.AppError('Student not found', 404);
    }
    const student = result.rows[0];
    const parentsResult = await (0, connection_1.query)(`SELECT sp.id, sp.relationship_type, sp.is_primary,
     ...
     ORDER BY sp.is_primary DESC, u.first_name`, [student.id]);
    ...
    const historyResult = await (0, connection_1.query)(`SELECT sch.start_date, sch.end_date,
            c.name as class_name, c.grade, c.section,
            ay.name as academic_year_name
     ...
     ORDER BY sch.start_date DESC`, [student.id]);
    const classHistory = historyResult.rows.map((history) => ({
...
exports.updateStudent = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (updateData.classId) {
            const classResult = await client.query('SELECT id, capacity, current_enrollment FROM classes WHERE id = $1 AND is_active = true', [updateData.classId]);
            if (classResult.rows.length === 0) {
                throw new errorHandler_1.AppError('Class not found or inactive', 404);
            }
            const classInfo = classResult.rows[0];
            ...
            await client.query('UPDATE student_class_history SET end_date = CURRENT_DATE WHERE student_id = $1 AND end_date IS NULL', [actualStudentId]);
            await client.query(`INSERT INTO student_class_history (student_id, class_id, academic_year_id, start_date)
         SELECT $1, $2, c.academic_year_id, CURRENT_DATE
         FROM classes c WHERE c.id = $2`, [actualStudentId, updateData.classId]);
        }
        ...
        if (studentUpdateFields.length > 0) {
            ...
            await client.query(`UPDATE students SET ${studentUpdateFields.join(', ')}, updated_at = CURRENT_TIMESTAMP
         WHERE id = $${studentParamCount}`, studentValues);
        }
        if (userUpdateFields.length === 0 && studentUpdateFields.length === 0) {
            throw new errorHandler_1.AppError('No fields to update', 400);
        }
        const updatedResult = await client.query(`SELECT s.id, s.alt_id, s.user_id, s.student_id, s.class_id, s.enrollment_date,
              s.guardian_name, s.guardian_phone, s.guardian_email, s.emergency_contact,
              s.medical_info, s.is_active, s.created_at, s.updated_at,
              u.first_name, u.last_name, u.email, u.phone, u.date_of_birth, u.address,
              c.name as class_name, c.grade, c.section,
...
exports.deleteStudent = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    let student;
    if (isUUID) {
        student = await (0, connection_1.query)('SELECT id, user_id FROM students WHERE id = $1', [id]);
    }
    else {
        student = await (0, connection_1.query)('SELECT id, user_id FROM students WHERE alt_id = $1 OR id::text = $1 OR student_id = $1', [id]);
    }
    ...
    await (0, connection_1.query)('UPDATE students SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1', [actualStudentId]);
    await (0, connection_1.query)('UPDATE users SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1', [userId]);
    res.json({
        success: true,
...
exports.getStudentClassHistory = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    let student;
    if (isUUID) {
        student = await (0, connection_1.query)('SELECT id FROM students WHERE id = $1', [id]);
    }
    else {
        student = await (0, connection_1.query)('SELECT id FROM students WHERE alt_id = $1 OR id::text = $1 OR student_id = $1', [id]);
    }
    if (student.rows.length === 0) {
        throw new errorHandler_1.AppError('Student not found', 404);
    }
    const actualStudentId = student.rows[0].id;
    const historyResult = await (0, connection_1.query)(`SELECT sch.id, sch.start_date, sch.end_date, sch.created_at, sch.updated_at,
            c.name as class_name, c.grade, c.section,
     ...
     JOIN academic_years ay ON sch.academic_year_id = ay.id
     WHERE sch.student_id = $1
     ORDER BY sch.start_date DESC`, [actualStudentId]);
    const classHistory = historyResult.rows.map((history) => ({
        id: history.id,
        startDate: history.start_date,
        endDate: history.end_date,
...
exports.getStudentsByClass = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const classExists = await (0, connection_1.query)('SELECT id, name, grade, section FROM classes WHERE id = $1', [classId]);
    if (classExists.rows.length === 0) {
        throw new errorHandler_1.AppError('Class not found', 404);
    }
    const classInfo = classExists.rows[0];
    const countResult = await (0, connection_1.query)('SELECT COUNT(*) FROM students WHERE class_id = $1 AND is_active = true', [classId]);
    const total = parseInt(countResult.rows[0].count);
    const result = await (0, connection_1.query)(`SELECT s.id, s.alt_id, s.student_id, s.enrollment_date,
     ...
     LIMIT $2 OFFSET $3`, [classId, Number(limit), offset]);
    const students = result.rows.map((student) => ({
        id: student.id,
        altId: student.alt_id,
        studentId: student.student_id,
...
exports.bulkUpdateStudents = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!studentIds || !Array.isArray(studentIds) || studentIds.length === 0) {
        throw new errorHandler_1.AppError('Student IDs array is required', 400);
    }
    if (!updateData || Object.keys(updateData).length === 0) {
        throw new errorHandler_1.AppError('Update data is required', 400);
    }
    const client = await (0, connection_1.getClient)();
    const results = [];
    const errors = [];
    try {
        ...
        for (const studentId of studentIds) {
            try {
                const studentExists = await client.query('SELECT id, user_id FROM students WHERE id = $1', [studentId]);
...
exports.getStudentSummary = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    let studentResult;
    if (isUUID) {
        studentResult = await (0, connection_1.query)(`SELECT s.id, s.student_id, s.enrollment_date, s.guardian_name, s.guardian_phone, s.guardian_email, s.emergency_contact,
              u.first_name, u.last_name, u.email, u.phone, u.date_of_birth, u.address,
              c.name as class_name, c.grade, c.section,
              ay.name as academic_year_name
       FROM students s
       ...
       JOIN classes c ON s.class_id = c.id
       JOIN academic_years ay ON c.academic_year_id = ay.id
       WHERE (s.alt_id = $1 OR s.id::text = $1 OR s.student_id = $1) AND s.is_active = true`, [id]);
    }
    ...
    const statsResult = await (0, connection_1.query)(`SELECT 
       ...
       (SELECT MAX(created_at) FROM grades WHERE student_id = $1) as last_grade_date`, [student.id]);
