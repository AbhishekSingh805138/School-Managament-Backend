...
exports.createAcademicYear = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const existingYear = await (0, connection_1.query)('SELECT id FROM academic_years WHERE name = $1', [academicYearData.name]);
    ...
    if (academicYearData.isActive) {
        await (0, connection_1.query)('UPDATE academic_years SET is_active = false WHERE is_active = true');
    }
    const seqIdResult = await (0, connection_1.query)('SELECT generate_sequential_id($1) as next_id', ['academic_years']);
    const sequentialId = seqIdResult.rows[0].next_id;
    const result = await (0, connection_1.query)(`INSERT INTO academic_years (name, start_date, end_date, is_active, alt_id)
     VALUES ($1, $2, $3, $4, $5)
...
exports.getAcademicYears = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const countResult = await (0, connection_1.query)(`SELECT COUNT(*) FROM academic_years ${whereClause}`, queryParams);
    const total = parseInt(countResult.rows[0].count);
    const result = await (0, connection_1.query)(`SELECT id, alt_id, name, start_date, end_date, is_active, created_at, updated_at
     FROM academic_years 
     ${whereClause}
...
exports.getAcademicYearById = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    let result;
    if (isUUID) {
        result = await (0, connection_1.query)(`SELECT id, alt_id, name, start_date, end_date, is_active, created_at, updated_at
       FROM academic_years WHERE id = $1`, [id]);
    }
    else {
        result = await (0, connection_1.query)(`SELECT id, alt_id, name, start_date, end_date, is_active, created_at, updated_at
       FROM academic_years WHERE alt_id = $1 OR id::text = $1`, [id]);
    }
    if (result.rows.length === 0) {
        throw new errorHandler_1.AppError('Academic year not found', 404);
    }
    const academicYear = result.rows[0];
    const semestersResult = await (0, connection_1.query)(`SELECT id, alt_id, name, start_date, end_date, is_active, created_at, updated_at
...
exports.updateAcademicYear = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    let existingYear;
    if (isUUID) {
        existingYear = await (0, connection_1.query)('SELECT id FROM academic_years WHERE id = $1', [id]);
    }
    else {
        existingYear = await (0, connection_1.query)('SELECT id FROM academic_years WHERE alt_id = $1 OR id::text = $1', [id]);
    }
    if (existingYear.rows.length === 0) {
        throw new errorHandler_1.AppError('Academic year not found', 404);
    }
    ...
    if (updateData.isActive) {
        await (0, connection_1.query)('UPDATE academic_years SET is_active = false WHERE is_active = true AND id != $1', [actualYearId]);
    }
    const updateFields = [];
    const values = [];
    let paramCount = 1;
    ...
    const result = await (0, connection_1.query)(`UPDATE academic_years SET ${updateFields.join(', ')}, updated_at = CURRENT_TIMESTAMP
     WHERE id = $${paramCount}
     RETURNING id, alt_id, name, start_date, end_date, is_active, created_at, updated_at`, values);
    const academicYear = result.rows[0];
    res.json({
        success: true,
...
exports.deleteAcademicYear = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    let academicYear;
    if (isUUID) {
        academicYear = await (0, connection_1.query)('SELECT id, name FROM academic_years WHERE id = $1', [id]);
    }
    else {
        academicYear = await (0, connection_1.query)('SELECT id, name FROM academic_years WHERE alt_id = $1 OR id::text = $1', [id]);
    }
    ...
    await (0, connection_1.query)('DELETE FROM academic_years WHERE id = $1', [actualYearId]);
    res.json({
        success: true,
